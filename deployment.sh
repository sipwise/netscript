...
if checkBootParam debootstrapkeyring ; then
  GPG_KEYRING=$(getBootParam debootstrapkeyring)
fi
## }}}
...
if [ -z "${GPG_KEYRING}" ] ; then
  ...
  if ! gpg --keyserver pool.sks-keyservers.net --recv-keys "${GPG_KEYRING}" ; then
    die "Failed for fetch GPG key '${GPG_KEYRING}'"
  fi

  if ! gpg -a --export "${GPG_KEYRING}" | apt-key add - ; then
    die "Failed for import GPG key '${GPG_KEYRING}' as apt-key"
  fi
fi
