...
install_sipwise_key() {
  ...
  for x in 1 2 3; do
    ...
    if "$PRO_EDITION" ; then
      wget -O /etc/apt/trusted.gpg.d/sipwise.gpg https://${SIPWISE_REPO_HOST}/sppro/sipwise.gpg
    else
      wget -O /etc/apt/trusted.gpg.d/sipwise.gpg https://${SIPWISE_REPO_HOST}/spce/sipwise.gpg
    fi
...
fai_upgrade() {
  ...
  echo "deb https://${SIPWISE_REPO_HOST}/wheezy-backports wheezy-backports main" >> "$debsrcfile"
...
grml_debootstrap_upgrade() {
  ...
  if dpkg --compare-versions $present_version lt $required_version ; then
    ...
    mkdir -p "${TMPDIR}/statedir/lists/partial" "${TMPDIR}/cachedir/archives/partial"
    local debsrcfile=$(mktemp)
    echo "deb https://${SIPWISE_REPO_HOST}/grml.org grml-testing main" >> "$debsrcfile"
...
install_vbox_package() {
  ...
  echo "deb https://${DEBIAN_REPO_HOST}/debian/ wheezy-backports non-free" > \
...
ensure_augtool_present() {
  ...
  mkdir -p "${TMPDIR}/etc/preferences.d" "${TMPDIR}/statedir/lists/partial" \
    "${TMPDIR}/cachedir/archives/partial"
  echo "deb https://${DEBIAN_REPO_HOST}/debian/ wheezy main" > \
    "${TMPDIR}/etc/sources.list"
...
MIRROR="https://${DEBIAN_REPO_HOST}/debian/"
...
if [ "$DEBIAN_RELEASE" = "jessie" ] ; then
  ...
  SEC_MIRROR="https://${DEBIAN_REPO_HOST}/debian-security/"
...
get_installer_path() {
  if [ -z "$SP_VERSION" ] && ! $TRUNK_VERSION ; then
    ...
    if "$PRO_EDITION" ; then
      INSTALLER_PATH="https://${SIPWISE_REPO_HOST}/sppro/"
    else
      INSTALLER_PATH="https://${SIPWISE_REPO_HOST}/spce/"
    fi
  ...
  if "$PRO_EDITION" ; then
    ...
    local repos_base_path="https://${SIPWISE_REPO_HOST}/sppro/${SP_VERSION}/dists/${DEBIAN_RELEASE}/main/binary-amd64/"
    INSTALLER_PATH="https://${SIPWISE_REPO_HOST}/sppro/${SP_VERSION}/pool/main/n/ngcp-installer/"
  else
    local installer_package='ngcp-installer-ce'
    local repos_base_path="https://${SIPWISE_REPO_HOST}/spce/${SP_VERSION}/dists/${DEBIAN_RELEASE}/main/binary-amd64/"
    INSTALLER_PATH="https://${SIPWISE_REPO_HOST}/spce/${SP_VERSION}/pool/main/n/ngcp-installer/"
  fi

  # use a separate repos for trunk releases
  if $TRUNK_VERSION ; then
    local repos_base_path="https://${SIPWISE_REPO_HOST}/autobuild/dists/release-trunk-${DEBIAN_RELEASE}/main/binary-amd64/"
    INSTALLER_PATH="https://${SIPWISE_REPO_HOST}/autobuild/pool/main/n/ngcp-installer/"
  fi

  if [ -n "$NGCP_PPA_INSTALLER" ] ; then
    local repos_base_path="https://${SIPWISE_REPO_HOST}/autobuild/dists/${NGCP_PPA_INSTALLER}/main/binary-amd64/"
    INSTALLER_PATH="https://${SIPWISE_REPO_HOST}/autobuild/pool/main/n/ngcp-installer/"
  fi
...
deb [arch=amd64] https://${SIPWISE_REPO_HOST}/autobuild/release/release-${AUTOBUILD_RELEASE} release-${AUTOBUILD_RELEASE} main
EOF
  fi
}
