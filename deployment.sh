...
if [ "$DISK" = "vda" ] ; then
  case "$DEBIAN_RELEASE" in
    lenny|squeeze|wheezy)
      echo  "Applying /dev/disk/by-id/lvm-pv-uuid-* workaround for virtio disk and older Debian release"
      logit "Applying /dev/disk/by-id/lvm-pv-uuid-* workaround for virtio disk and older Debian release"
