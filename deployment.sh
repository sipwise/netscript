...
puppet_install_from_git () {
  ...
  case "${DEBIAN_RELEASE}" in
    jessie|stretch)
      ...
      grml-chroot $TARGET puppet apply --test --modulepath="${PUPPET_LOCAL_GIT}" --tags core 2>&1 | tee -a /tmp/puppet.log
      check_puppet_rc "${PIPESTATUS[0]}" "2"
      grml-chroot $TARGET puppet apply --test --modulepath="${PUPPET_LOCAL_GIT}" 2>&1 | tee -a /tmp/puppet.log
      check_puppet_rc "${PIPESTATUS[0]}" "2"
      ;;
  esac
}
