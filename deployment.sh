...
if "$PRO_EDITION" ; then
  ...
  case "$SP_VERSION" in
    ...
    3.0)     INSTALLER_VERSION="0.11.7" ; DEBIAN_RELEASE="wheezy" ;;
...
EOT
  ...
  if ! grml-chroot $TARGET dkms status | grep -q ngcp-mediaproxy-ng ; then
    ...
    if grml-chroot $TARGET dkms status | grep -q '^ngcp-mediaproxy-ng.*: installed' ; then
      ...
      KERNELHEADERS=$(basename $(ls -d ${TARGET}/usr/src/linux-headers*amd64 | sort -u | tail -1))
