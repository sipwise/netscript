...
echo "ngcp-installer exit code(s): ${PIPESTATUS[@]}"
RC=${PIPESTATUS[0]:-42}
if [ "${RC}" != "0" ] ; then
  echo "ERROR: Fatal error while running ngcp-installer (exit code '${RC}')!" >&2
  exit ${RC}
fi
echo "All OK, ngcp-installer finished with exit code '${RC}', continue netscript deployment."
EOT
