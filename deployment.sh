...
install_vbox_iso() {
  ...
  local vbox_version=$(curl -L -s http://download.virtualbox.org/virtualbox/LATEST.TXT)
  if [ -z "$vbox_version" ] ; then
    die "Error: Failed to identify VirtualBox version (no network access to http://download.virtualbox.org/?). Exiting."
  fi

  local vbox_iso="$(curl -L -s http://download.virtualbox.org/virtualbox/${vbox_version}/SHA256SUMS | grep 'iso$')"
  local vbox_checksum="${vbox_iso/ */}"
  local vbox_iso="${vbox_iso//* *VBoxGuestAdditions/VBoxGuestAdditions}"
