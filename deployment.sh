...
EOT
  ...
  cat << "EOT" | grml-chroot $TARGET /bin/bash
set +e
echo "Running ngcp-installer into grml-chroot..." | tee -a /tmp/ngcp-installer-debug.log
ngcp-installer 2>&1 | tee -a /tmp/ngcp-installer-debug.log
CHECK=(${PIPESTATUS[@]})
echo "ngcp-installer finished with PIPESTATUS='${CHECK[@]}'" | tee -a /tmp/ngcp-installer-debug.log
RC=${CHECK[0]:-42}
if [ "${RC}" != "0" ] ; then
